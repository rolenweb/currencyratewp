...
(function($,window){
    ...
    $('div.post-entry').on('change', 'select[name = "code-from"], div[name = "amount"] input, select[name = "code-to"]',function( event ) {  
    	calculateRate();
    });

    function calculateRate() {
    	var code_from = $('div.post-entry').find('select[name = "code-from"]').val();
    	var code_to = $('div.post-entry').find('select[name = "code-to"]').val();
    	var result = $('div.post-entry').find('div[name = "value"] span');
    	var value = parseFloat($('div.post-entry').find('div[name = "amount"] input').val());
    	var rate = parseFloat(getRate(code_from,code_to));
    	result.text((value*rate).toFixed(2));
    }
    ...
    function ratesValue() {
    	var value = [];
    	for (elem in rates) {
		   value.push(rates[elem]);
		}
		return value;
    }

    function getRate(from,to) {
    	var rates = ratesValue();
    	var codes = ratesCode();
    	...
    	if (from === 'USD') {
    		...
    		return parseFloat(t_to)/parseFloat(t_from);
    	}
    }
